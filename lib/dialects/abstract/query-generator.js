...
var QueryGenerator = {
  ...
  */
  insertQuery: function(table, valueHash, modelAttributes, options) {
    ...
    var query, valueQuery = '<%= tmpTable %>INSERT<%= ignore %> INTO <%= table %> (<%= attributes %>)<%= output %> VALUES (<%= values %>)',
    ...
    if (this._dialect.supports.returnValues && options.returning) {
      if (!!this._dialect.supports.returnValues.returning) {
        ...
        if (modelAttributes && options.hasTrigger && this._dialect.supports.tmpTableTrigger) {
          ...
          tmpTable = Utils._.template(tmpTable)(replacement)
            .trim();
  ...
  */
  updateQuery: function(tableName, attrValueHash, where, options, attributes) {
    ...
    var query, values = [],
      outputFragment, modelAttributeMap = {},
      tmpTable = '' //tmpTable declaration for trigger
      ,
      selectFromTmp = '' //Select statement for trigger
      ,
  ...
  */
  addIndexQuery: function(tableName, attributes, options, rawTablename) {
    ...
    ind = ind.concat(
      ...
      options.type, 'INDEX', !this._dialect.supports.indexViaAlter ? concurrently : undefined,
      this.quoteIdentifiers(options.name),
      this._dialect.supports.index.using === 1 && options.using ? 'USING ' + options.using : '', !this._dialect.supports.indexViaAlter ? 'ON ' + tableName : undefined,
  ...
  */
  quote: function(obj, parent, force) {
    if (Utils._.isString(obj)) {
      ...
      var tableNames = [],
        parentAssociation, len = obj.length,
        item, model, as, association;
  ...
  */
  quoteIdentifiers: function(identifiers, force) {
    if (identifiers.indexOf('.') !== -1) {
      ...
      return this.quoteIdentifier(identifiers.slice(0, identifiers.length - 1)
        .join('.')) + '.' + this.quoteIdentifier(identifiers[identifiers.length - 1]);
    } else {
  ...
  */
      ...
      var generateJoinQueries = function(include, parentTable) {
        ...
        if (through) {
          ...
          , tableRight = as, attrRight = association.associationType !== 'BelongsTo' ?
            association.identifierField || association.identifier :
            right.rawAttributes[association.targetIdentifier || primaryKeysRight[0]].field, joinOn, subQueryJoinOn;
          ...
          if (subQuery && association.isMultiAssociation && include.required) {
            ...
            if (options.where instanceof Utils.and) {
              ...
              options.where = {
                $and: [options.where, subQueryWhere]
              };
            }
          }
  ...
  whereItemQuery: function(key, value, options) {
    ...
    var self = this,
      binding, outerBinding, comparatorMap, aliasMap, comparator = '=',
      ...
      return (function() {
        ...
        _.forOwn(value, function(item, prop) {
          if (prop.indexOf('$') === 0) {
            var $where = {};
            $where[prop] = item;
            $items.push(self.whereItemQuery(key, $where, _.assign({}, options, {
              json: false
            })));
            return;
          }
    ...
    if (_.isPlainObject(value) && Object.keys(value)
      .length > 1) {
      return (function() {
        var $items = [];
    ...
    if (Array.isArray(value) && fieldType instanceof DataTypes.ARRAY) {
      ...
      if (value.$any && value.$any.$values || value.$all && value.$all.$values) {
        value = '(VALUES ' + (value.$any && value.$any.$values || value.$all && value.$all.$values)
          .map(function(value) {
      ...
      value = (value.$between || value.$notBetween)
        .map(function(item) {
          return self.escape(item);
        })
        .join(' AND ');
    ...
    if (comparator === 'REGEXP_LIKE') {
      return 'REGEXP_LIKE(' + key + ',' + value + ', \'i\')';
    }
