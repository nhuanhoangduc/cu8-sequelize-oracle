...
module.exports = (function() {
    ...
    Query.prototype.formatResults = function(data, done) {
        ...
        // Read clob
        if (this.clobAttributes && this.isSelectQuery()) {
            if (!result) {
                return done(null, null);
            }

            if (util.isArray(result)) {
                async.eachLimit(result, 5, function(item, nextItem) {

                    async.each(_this.clobAttributes, function(key, nextKey) {

                        var lob = item.dataValues[key];
