...
module.exports = (function() {
    ...
    Query.prototype.formatResults = function(data, done) {
        ...
        if (this.isSelectQuery()) {
            //substitute column aliases back
            let aliases=this.options.aliases.aliasMapNormalized;
            //data.rows
            let normalizedRows=data.rows.map((row)=>{
                let normalizedRow={};
                for(let key of Object.keys(row)){
                    let finalKey = key;
                    let splittedKey = key.split('.');
                    let keyPrefix = splittedKey[0];
                    let alias = aliases.find((a)=>{
                        return a.sas === keyPrefix;
                    });
                    if(alias !== undefined){
                        //get normalized key
                        finalKey = [].concat(alias.key,splittedKey.slice(1)).join('.');
                    }
                    normalizedRow[finalKey] = row[key];
                }
                return normalizedRow;
            });
            result = this.handleSelectQuery(normalizedRows);
    ...
    AbstractQuery.prototype.handleInsertQuery = function(results, metaData) {
        ...
        if (this.instance) {
            ...
            // console.log("\n\natts");
            // console.log(autoIncrementField);
            // console.log(id);
            ...
            // console.log("---");
            // console.log(this.getInsertIdField());
            // console.log("\n\n\n\n\n");
            
            if (results && results.outBinds && results.outBinds[this.getInsertIdField()]) {
                id = results.outBinds[this.getInsertIdField()];
            }
