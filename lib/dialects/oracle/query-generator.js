...
module.exports = (function() {
  var QueryGenerator = {
    ...
    insertQuery: function(table, valueHash, modelAttributes, options) {
      ...
      var query, valueQuery = 'INSERT<%= ignore %> INTO <%= table %> (<%= attributes %>)<%= output %> VALUES (<%= values %>)',
        emptyQuery = 'INSERT<%= ignore %> INTO <%= table %><%= output %>',
        outputFragment, fields = [],
        values = [],
        clobColumns = [],
        key, value, identityWrapperRequired = false,
        modelAttributeMap = {},
        bindParameters = {};
      ...
      if (modelAttributes) {
        Utils._.each(modelAttributes, function(attribute, key) {
          ...
          if (String(attribute.type).match("CLOB")) {
            clobColumns.push(key);
          }
        });
      }

      if (clobColumns.length) {
        var valueQuery = `DECLARE ${Utils._.map(clobColumns, clobColumn => {
          var value = valueHash[clobColumn];
          var valueChunks = Utils._.map(Utils._.chunk(value, 30000), arr => {
            return arr.join('');
          });
          return Utils._.map(valueChunks, (valueChunk, vidx) => {
            return `${clobColumn}_val_${vidx} CLOB:= '${valueChunk}';`
          }).join('');
        }).join('')} BEGIN INSERT<%= ignore %> INTO <%= table %> (<%= attributes %>)<%= output %> VALUES (<%= values %>)`;
      } else {
        var valueQuery = 'INSERT<%= ignore %> INTO <%= table %> (<%= attributes %>)<%= output %> VALUES (<%= values %>)';
      }
      ...
      for (key in valueHash) {
        ...
        if (valueHash.hasOwnProperty(key)) {
          ...
          if (modelAttributeMap && modelAttributeMap[key] && modelAttributeMap[key].autoIncrement === true && !value) {
            ...
            if (clobColumns.includes(key)) {
              values.push(`${key}_val`);
            }
            ...
            if (this._dialect.supports.returnValues && options.returning) {
              if (!!this._dialect.supports.returnValues.returning) {
                valueQuery += ' RETURNING ' + this.quoteIdentifier(key) + ' INTO :rid' + (clobColumns.length ? ';END' : '');
            ...
            if (clobColumns.includes(key)) {
              var num = Math.ceil(value.length / 30000);
              var str = [];
              for (var i = 0; i < num; i++) {
                str.push(`${key}_val_${i}`);
              }
    ...
    selectQuery: function(tableName, options, model) {
      ...
      var table = null,
        ...
        subJoinQueries = [],
        mainTableAs = null,
        shortAliasMap=[],  //shorten large alias names got from table names concatenation, format: shortAliasMap[i]={key:"largeAlias",sas:"shortAlias"}
        shortAliasMapNormalized=[],
        sAsId=0;

        const normalizeSAsMap = function(unnormalized){
            let raw = unnormalized;//make working copy of array!
            return [].concat(
                unnormalized[0],//first element is normalized by default
                //start from second element
                unnormalized.slice(1).map((value)=>{
                    let splittedKey = value.key.split('.');
                    let substitutablePart=splittedKey[0];// === "t0" for example
                    let sasObj = raw.find((r)=>{
                        return r.sas === substitutablePart;
                    });
                    let realKeyPrefix = sasObj.key;
                    let finalSAsObj={};
                    if(realKeyPrefix !== undefined){
                        let finalKey = [].concat(realKeyPrefix,splittedKey.slice(1)).join('.');
                        finalSAsObj = {
                            key: finalKey,
                            sas: value.sas
                        };

                        //then substitute and update raw array
                        raw=raw.map((r)=>{
                            return (r.sas === value.sas)?finalSAsObj:r;
                        });
                    }

                    //and return final sas object
                    return finalSAsObj;
                })
            )
        };

        const getSAs = function(longAlias){// as - large alias
            let sasObj = shortAliasMap.find((value)=>{
                return value.key === longAlias;
            });
            if(sasObj === undefined){
                shortAliasMap.push({
                    key: longAlias,
                ...
                sasObj = shortAliasMap[shortAliasMap.length - 1];
            };
            //keep normalized map updated!
            shortAliasMapNormalized=normalizeSAsMap(shortAliasMap);
            return sasObj.sas;
        };
      ...
      if (options.include) {
        var generateJoinQueries = function(include, parentTable) {
          var table = include.model.getTableName(),
            ...
            attributes, association = include.association,
          ...
          if (tableName !== parentTable && mainTableAs !== parentTable) {
              as = getSAs(parentTable + '.' + include.as);
          }
          ...
          if (through) {
            var throughTable = through.model.getTableName(),
              throughAs = getSAs(as + '.' + through.as),
              throughAttributes = through.attributes.map(function(attr) {
                return self.quoteIdentifier(throughAs) + '.' + self.quoteIdentifier(Array.isArray(attr) ? attr[0] : attr) +
                  ' ' +
                  self.quoteIdentifier(throughAs + '.' + (Array.isArray(attr) ? attr[1] : attr),true);
              }),
            ...
            , tableRight = as, attrRight = association.associationType !== 'BelongsTo' ?
              association.identifierField || association.identifier :
              right.rawAttributes[primaryKeysRight[0]].field || primaryKeysRight[0], joinOn, subQueryJoinOn;
      ...
      const substituteSAs = function(longName){
          const longNameSplitted = longName.split('.');
          const longPrefix = longNameSplitted.slice(0,-1).join('.');
          let shortPrefix = longPrefix;
          const mapping = shortAliasMapNormalized.find((value)=>{return value.key === longPrefix;});
          if(mapping !== undefined){
              shortPrefix = mapping.sas;
          }
          const shortName = shortPrefix+'.'+longNameSplitted.slice(-1);
          return shortName;
      };
      ...
      if (options.order) {
        ...
        if (Array.isArray(options.order)) {
          options.order.forEach(function(t) {
            ...
            t[0].val=substituteSAs(t[0].val);//correct aliases in order statements
      ...
      return {sql:query, bind:{},
          aliases:{
              aliasMap: shortAliasMap,
              aliasMapNormalized: shortAliasMapNormalized
          }
      };
    },
